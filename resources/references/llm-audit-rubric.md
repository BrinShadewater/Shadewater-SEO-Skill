...
- `Provider scope`: Universal, Google-specific, Bing-specific, Regional, or Experimental
- `Evidence`: observable proof (HTML element, metric, URL, script output)
- `Evidence source`: browser/rendered page, raw HTML, script output, Search Console, Bing Webmaster Tools, provider docs, or manual review
- `Impact`: SEO consequence (indexing, ranking, CTR, UX, crawl efficiency)
- `Fix`: concrete implementation step
- `Action type`: Required, Recommended, Optional, or Experimental
...
- GEO/AI search readiness signals, scoped by provider
...
- Google AI Search requirements for `llms.txt`, AI-only schema, artificial chunking, or AI-specific rewrites
- Bing AI citation performance without Bing Webmaster Tools AI Performance or other explicit evidence
...
```text
[Area] <name>
Provider Scope: <Universal|Google-specific|Bing-specific|Regional|Experimental>
Severity: <Critical|Warning|Pass|Info>
Confidence: <Confirmed|Likely|Hypothesis>
Action Type: <Required|Recommended|Optional|Experimental>
Finding: <one sentence>
Evidence: <specific proof>
Evidence Source: <browser|raw HTML|script output|provider docs|Search Console|Bing Webmaster Tools|manual review>
Impact: <why this matters>
Fix: <clear implementation step>
```

## 11) Provider Scope And Action Type

Use provider scope to prevent overgeneralized SEO advice:

- `Universal`: basic web/search quality issue supported across providers, such as blocked important pages, broken canonicalization, inaccessible content, or invalid structured data.
- `Google-specific`: based on Google Search Central, Googlebot, Search Console, PageSpeed/CrUX, Google rich results, or Google generative AI guidance.
- `Bing-specific`: based on Bing Webmaster Guidelines, Bingbot, Bing Webmaster Tools, Bing AI Performance, or IndexNow guidance.
- `Regional`: applies to Yandex, Naver, Baidu, or another market-specific search provider.
- `Experimental`: emerging AI-search tactic, third-party study, or unverified provider behavior.

Use action type to describe how strongly the user should act:

- `Required`: eligibility, indexing, security, policy, or correctness blocker.
- `Recommended`: provider-backed best practice with likely user/search benefit.
- `Optional`: useful enhancement for a narrower workflow or lower-risk site.
- `Experimental`: worth testing only when the user explicitly wants emerging AI/search experiments.

## 12) Chain-of-Thought Scoring Protocol
