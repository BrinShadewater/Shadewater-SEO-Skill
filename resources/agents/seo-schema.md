...
|--------|---------|---------|
| `parse_html.py` | Extract JSON-LD blocks + validate types | `python parse_html.py --url <url> --json` |
| `validate_schema.py` | Post-edit validation (pre-commit) | `python validate_schema.py <file>` |
| `entity_checker.py` | Entity sameAs and KG presence | `python entity_checker.py <url> --json` |
...
| Type | Restriction |
|------|------------|
| `FAQPage` | FAQ rich results fully retired May 7, 2026 — no site type qualifies anymore. Existing markup is harmless but earns nothing; do not recommend adding it. |
