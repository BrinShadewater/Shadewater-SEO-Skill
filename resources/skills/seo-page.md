...
- For client-rendered pages: verify the raw HTML shell already includes the H1, core copy, internal links, and structured data needed by non-rendering crawlers.
...
- `seo-image-handoff.json` — Auto-generated when `generate_report.py` finds actionable image issues on the page

When `generate_report.py` is used, the HTML dashboard and both markdown files are emitted in one pass so the page audit has machine-readable and human-readable deliverables by default.
...
6. If the target is a preview/staging host and secondary fetches return 401, label those failures as environment limitations and rerun final scoring on a public production URL.
