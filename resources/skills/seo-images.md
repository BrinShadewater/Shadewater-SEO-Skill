...
## WebP Remediation Bridge

When the user wants the image findings turned into concrete asset work, generate a handoff file for `webp-me-daddy` instead of leaving the audit as advice only.
...
Use this bridge only when the image src values can be mapped to real files under the local `public/` folder. If the handoff marks an item as `manual`, keep it as a human-reviewed task instead of forcing it through the still-image pipeline.
