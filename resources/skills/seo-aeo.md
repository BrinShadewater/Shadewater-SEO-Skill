...
AEO is a workflow label for answer-oriented search features such as Featured Snippets, People Also Ask, Knowledge Panels, and Sitelinks Searchbox. It is not a separate replacement for SEO. Use official provider docs first, and keep Google/Bing/regional recommendations scoped.

For Google generative AI features, read `resources/references/google-ai-optimization-guide.md`: Google treats AI Search visibility as an extension of normal Search quality, crawlability, indexability, snippet eligibility, helpful content, and page experience. Do not recommend `llms.txt`, special AI schema, or artificial chunking as Google AEO/GEO requirements.
...
| **AI Overview / AI Mode** (GEO) | Google Search index and quality systems | Foundational SEO, snippet eligibility, helpful unique content, clear structure |
...
- [ ] Do **not** add `FAQPage` schema — FAQ rich results were fully retired May 7, 2026 (the earlier gov/healthcare carve-out is gone). Use visible Q&A content and appropriate `Article`/`WebPage` schema when it matches the page
...
Each finding should include provider scope:

- `Google-specific` for Featured Snippet, PAA, Knowledge Panel, rich-result, or AI Search guidance grounded in Google docs.
- `Bing-specific` when using Bing Webmaster Tools or Bing AI Performance data.
- `Experimental` for third-party AI citation tactics without primary provider support.
