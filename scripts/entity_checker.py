...
SAMEAS_PLATFORMS = {
    ...
    "twitch.tv": {"name": "Twitch", "priority": "Medium", "kg_signal": "Moderate"},
    "letterboxd.com": {"name": "Letterboxd", "priority": "Low", "kg_signal": "Weak"},
    "facebook.com": {"name": "Facebook", "priority": "Low", "kg_signal": "Weak"},
    "instagram.com": {"name": "Instagram", "priority": "Low", "kg_signal": "Weak"},
}

SOCIAL_VERIFICATION_BLOCKLIST = {
    "x.com",
    "twitter.com",
    "twitch.tv",
    "letterboxd.com",
...
def analyze_sameas(same_as_list: list) -> dict:
    ...
    # Check for broken URLs (quick HEAD/GET check on first 3)
            ...
            try:
                ...
                with urllib.request.urlopen(req, timeout=6) as resp:
                    if resp.status < 400:
                        verification_error = None
                        break
                    verification_error = ("http", resp.status)
            except urllib.error.HTTPError as exc:
                verification_error = ("http", exc.code)
                if method == "HEAD" and exc.code in (401, 403, 405):
                    continue
                break
            except Exception as exc:
                verification_error = ("exception", str(exc))
                break

        if verification_error is None:
            continue

        kind, detail = verification_error
        if kind == "http" and detail in (401, 403, 405) and domain in SOCIAL_VERIFICATION_BLOCKLIST:
            continue

        if kind == "http":
            issues.append({
                "severity": "Warning",
                "finding": f"sameAs URL returns HTTP {detail}: {url}",
