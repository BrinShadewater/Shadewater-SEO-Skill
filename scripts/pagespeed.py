...
def get_pagespeed(url: str, strategy: str = "mobile", api_key: str = None) -> dict:
    ...
    if data is None:
        result["error"] = "No PageSpeed data returned."
        return result
...
def resolve_api_key(explicit_api_key: str | None = None) -> str | None:
    """Prefer an explicit key, then fall back to common environment variables."""
    if explicit_api_key:
...
def main():
    ...
    result = get_pagespeed(args.url, strategy=args.strategy, api_key=resolve_api_key(args.api_key))
