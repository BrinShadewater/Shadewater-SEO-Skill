...
from net_utils import validate_public_url
...
def is_probably_bot_blocked(link: dict, status: int | None) -> bool:
    """Treat external 401/403 responses as manual-review items, not hard broken links."""
    return bool(status in {401, 403} and not link.get("is_internal"))
...
def check_link(link: dict, timeout: int = 10) -> dict:
    ...
    try:
        ...
        result["error"] = "blocked_private_target"
...
def check_broken_links(url: str, internal_only: bool = False,
    ...
    for link in checked:
        ...
        elif is_probably_bot_blocked(link, status):
            result["blocked"].append(link)
    ...
    if result["blocked"]:
        result["issues"].append(
            f"ℹ️ {len(result['blocked'])} external link(s) returned 401/403 and may be bot-blocked"
        )
...
def main():
    ...
    print(f"Total: {s['total']} | ✅ Healthy: {s['healthy']} | "
          f"🔴 Broken: {s['broken']} | 🟡 Blocked: {s['blocked']} | ↪️ Redirected: {s['redirected']} | "
    ...
    if result["blocked"]:
        ...
        for link in result["blocked"]:
            print(f"  [{link['status']}] (external) {link['url']}")
            print(f"         anchor: \"{link['anchor_text']}\"")
