...
def _validate_schema_object(obj: dict, block_num: int) -> List[str]:
    ...
    restricted = {"FAQPage": "no longer eligible for rich results (fully retired May 2026)"}
    if schema_type in restricted:
        errors.append(f"{prefix}: @type '{schema_type}' is {restricted[schema_type]} — markup is harmless but earns nothing; do not add to new pages")
