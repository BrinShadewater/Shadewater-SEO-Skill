...
$REPO_URL = if ($env:REPO_URL) { $env:REPO_URL } else { 'https://github.com/BrinShadewater/Shadewater-SEO-Skill.git' }
$SKILL_NAME = 'shadewater-seo'
$TARGET = 'claude'
...
function Show-Usage {
@'
Shadewater SEO Skill Installer (Claude / Codex / Antigravity)

Usage:
  pwsh ./install.ps1 [options]

Options:
  --target <claude|codex|global|antigravity|all> Install target (default: claude)
  --project-dir <path>                            Project path for antigravity target (default: current directory)
  --skill-name <name>                             Installed folder name (default: shadewater-seo)
...
Examples:
  ...
  pwsh ./install.ps1 --target claude --repo-path /path/to/checkout

  # Install from a custom remote repository URL
  pwsh ./install.ps1 --target codex --source remote --repo-url https://github.com/you/your-fork.git
'@ | Write-Host
}
